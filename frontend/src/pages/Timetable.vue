...
const canvas = useTemplateRef<HTMLCanvasElement>('timetable')
    ...
    onMounted(() => {
        ...
        let mouseX: number
        let mouseY: number
        let mouseHoverID: number
        
        // Variables for Selection for creating new Task
        let cursor_day: number
        let cursor_hour: number
        let cursor_select = {}
        ...
        async function setup() {
          if (canvas.value) {
            ...
            canvas.value.addEventListener("mousemove", mousemovement, false);
            canvas.value.addEventListener("mousedown", mousedown, false);
            canvas.value.addEventListener("mouseup", mouseup, false);
        ...
        function calculateMonday(reset = false) {
          ...
          if (php_date !== "" && (dateOfMonday === undefined || (dateOfMonday !== undefined && php_date !== dateOfMonday.toLocaleDateString('sv-SE'))))
            d = new Date(php_date)
        ...
        function print_canvas() {
          ...
          win?.document.write("<br><img src = '" + canvas.value?.toDataURL() + "'/>");
        ...
        function mousemovement(e) {
          mouseX = e.clientX - canvas.value.offsetLeft;
          mouseY = e.clientY - canvas.value.offsetTop;
          ...
          canvas.value.style.cursor = overHeader ? "pointer" : "default"
        }
